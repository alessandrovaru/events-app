...
export const ReservationsModal = ({classItem}) => {
  ...
  return (
    <>
      <button onClick={handleOpen} className="relative z-10 bg-white text-black text-xs px-3 py-1 rounded-lg hover:opacity-90">Reservar</button>
      {isOpen ? (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-20 !mt-0">
          <div className="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <div className="relative z-20 mt-3">
              <h3 className="text-lg leading-6 font-medium text-gray-900">Confirma tu reserva</h3>
              <h4 className="text-sm text-gray-500 mt-2">Detalles de la clase</h4>
              <p className="text-sm text-gray-500 mt-2"><span className="font-bold">Nombre de la clase:</span> {classItem.name}</p>
              <p className="text-sm text-gray-500 mt-2"><span className="font-bold">Día:</span> {classItem.day}</p>
              <p className="text-sm text-gray-500 mt-2"><span className="font-bold">Hora:</span> {classItem.time}</p>
              <p className="text-sm text-gray-500 mt-2"><span className="font-bold">Lugar:</span> {classItem.location}</p>
              <p className="text-sm text-gray-500 mt-2"><span className="font-bold">Precio:</span> {classItem.price || 'Gratis'}</p>
              <hr className="mt-2" />
              <div className="mt-2">
                <h4 className="text-sm text-gray-500">Confirma tu asistencia</h4>
                <form className="space-y-6" onSubmit={handleSubmit}>
                  <div>
                    <label htmlFor="payment_type" className="block text-gray-700 text-sm font-bold mb-2 mt-2">
                      Tipo de pago
                    </label>
                    <div className="mt-2">
                      <select
